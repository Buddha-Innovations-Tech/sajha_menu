...
MyPage = Class.extend({
	...
	refresh: async function () {
		...
		await frappe.call({
			method: "sajha_menu.sajha_menu.page.kitchen_order.kitchen_order.refreshKot",
	...
	filters: async function () {
		...
		async function changesTab() {
			...
			await frappe.call({
				method: "sajha_menu.sajha_menu.page.kitchen_order.kitchen_order.applyFilter",
				args: { "company": company, "fromDate": fromDate, "toDate": toDate, "status": status },
				callback: function (r) {
					$("#body-main")[0].innerHTML = frappe.render_template(r.message)
				}
		...
		await frappe.call({
			method: "sajha_menu.sajha_menu.page.kitchen_order.kitchen_order.getCompanyList",
			callback: function (r) {
	...
	make: async function () {
		...
		let body = await frappe.call({
			method: "sajha_menu.sajha_menu.page.kitchen_order.kitchen_order.getBodyHTML",
